...
export function generateWaterfallChart(input: ChartInput): string {
  ...
  const bars: Array<{
    label: string;
    barStart: number; // the "from" value (bottom of floating bar)
    barEnd: number;   // the "to" value (top of floating bar)
    isTotal: boolean;
    value: number;
    runningTotal: number;
  }> = [];
