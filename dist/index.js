...
const index_js_2 = require("./charts/index.js");
...
const TOOLS = [
    {
        name: "generate_chart",
        description: "Generate a beautiful, presentation-ready chart and return it as an SVG string. " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                data: {
                    type: "array",
                    description: "Data points. Shape varies by chart type — use list_chart_types or get_chart_schema for details.",
                    items: { type: "object" },
                },
                title: {
                    type: "string",
    ...
    },
    {
        ...
        description: "List all supported chart types with descriptions, required data shapes, and examples. " +
    ...
    },
    {
        ...
        description: "Get the full JSON schema for a specific chart type, including all required and optional fields.",
        inputSchema: {
            type: "object",
            properties: {
                type: {
    ...
    {
        name: "save_chart",
        description: "Save a chart to a file as SVG or PNG. " +
    ...
    {
        name: "describe_chart",
        description: "Given your data and context, recommend the best chart type and explain why. " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                context: {
                    type: "string",
                    description: "What you're trying to show or communicate. " +
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "generate_chart": {
                const input = args;
                const result = (0, index_js_2.generateChart)(input);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "list_chart_types": {
                return {
                    content: [
                        {
                            type: "text",
                            text: JSON.stringify(schemas_js_1.CHART_TYPE_INFO, null, 2),
                        },
                    ],
                };
            }
            case "get_chart_schema": {
                const { type } = args;
                const schema = schemas_js_1.CHART_SCHEMAS[type];
                if (!schema) {
                    const supported = Object.keys(schemas_js_1.CHART_SCHEMAS).join(", ");
            ...
            case "save_chart": {
                const { chartId, svg: svgInput, outputPath, format } = args;
                let svgContent;
                if (chartId) {
                    const cached = types_js_2.chartCache.get(chartId);
                    if (!cached) {
                        throw new Error(`Chart "${chartId}" not found in cache. ` +
                ...
                if (format === "svg") {
                    ...
                    try {
                        ...
                        throw new Error("PNG export requires the 'sharp' package. " +
...
function describeChart(data, context) {
    ...
    if (hasValues) {
        const seriesCount = first.values.length;
