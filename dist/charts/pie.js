...
function describeDonutArc(cx, cy, r, ir, startAngle, endAngle) {
    const end = endAngle - startAngle >= 360 ? startAngle + 359.99 : endAngle;
...
function buildPieOrDonut(input, isDonut) {
    ...
    if (total <= 0)
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}"><rect width="${W}" height="${H}" fill="${theme.background}"/><text x="${W / 2}" y="${H / 2}" text-anchor="middle" fill="${theme.subtext}" font-size="14">No data</text></svg>`;
...
function generatePieChart(input) {
    return buildPieOrDonut(input, false);
}
function generateDonutChart(input) {
